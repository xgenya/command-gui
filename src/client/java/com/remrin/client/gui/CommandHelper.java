...
/**
 * Shared utility methods for command execution, placeholder detection,
 * fake player identification, and time formatting.
 */
public final class CommandHelper {
	...
	/**
	 * Send a command or chat message. Leading "/" is stripped and sent via
	 * {@code sendCommand}; everything else is sent via {@code sendChat}.
	 */
	...
	public static boolean hasPlaceholders(List<String> commands) {
		...
		for (String cmd : commands) {
			if (hasPlaceholders(cmd)) return true;
		}
		return false;
	}
	...
	/**
	 * Detect if a player is a Carpet fake player based on 0 ping and not being
	 * the local player. This is the standard heuristic used by Carpet Mod.
	 */
	...
	/**
	 * Format total seconds into a descriptive duration string with units.
	 * Examples: "5s", "2m 30s", "1h 05m 30s", "--" for zero.
	 */
