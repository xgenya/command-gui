...
/**
 * Preset command tab, displaying read-only preset commands loaded from JSON files (e.g. vanilla
 * commands, Carpet commands).
 * <p>
 * Command data comes from {@link VanillaCommands#getGroups(String)}; each group maps to one
 * category button in the sidebar. Supports commands with parameter ranges (via the
 * {@code minValue}/{@code maxValue}/{@code quickValues} fields).
 */
public class PresetCommandTab extends AbstractCommandTab {
  ...
  /**
   * Executes a preset command: if the command has a numeric/time range, pops the input screen
   * first; otherwise executes it directly via {@link ChainedCommandExecutor}.
   */
  ...
  public int getCommandIndexAtPosition(double mouseX, double mouseY) {
    ...
    if (mouseX < cmdAreaLeft || mouseX > cmdAreaLeft + cmdAreaWidth) {
      return -1;
    }
    if (mouseY < area.top() || mouseY > area.bottom()) {
      return -1;
    }
